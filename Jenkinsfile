node {
  ...
  def appRepo = "mycr1257.azurecr.io/app:master.10"
 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  switch (env.BRANCH_NAME) {
    ...
    case "canary":
        ...
        sh("echo http://`kubectl --kubeconfig ~joanna/.kube/config --namespace=prod get service/${appName} --output=json | jq -r '.status.loadBalancer.ingress[0].ip'` > ${appName}")
        break
